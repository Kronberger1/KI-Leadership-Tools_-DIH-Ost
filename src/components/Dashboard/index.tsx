...
import { TrendingUp, TrendingDown, ChevronRight, Target, Award, BarChart3, CheckCircle2 } from 'lucide-react';
import { ProgressBar } from '../shared/ProgressBar';
import { ScoreDisplay } from '../shared/ScoreDisplay';
import { LEVEL_COLORS } from '../../data/competencies';
import type { AreaScore, EmployeeProfile, MaturityBadge } from '../../types';
...
function areaScoreToColor(score: number): string {
...
export const Dashboard: React.FC<Props> = ({
  ...
  const radarData = areaScores.map((s) => ({
    subject: s.areaTitle.replace('Copilot in ', '').replace('Copilot ', '').replace(' Copilot', ''),
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto view-transition">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm opacity-75">
              {profile.rolle} · {profile.abteilung}
            </p>
          </div>
      ...
      {!hasData && (
        <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-10 text-center mb-6">
          ...
          <h3 className="text-lg font-semibold text-gray-600 mb-2">Noch keine Bewertungen</h3>
      ...
      {hasData && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <h2 className="text-base font-semibold text-gray-800 mb-4">Kompetenz-Übersicht</h2>
          ...
          {/* Area progress cards */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <h2 className="text-base font-semibold text-gray-800 mb-4">Bereiche im Detail</h2>
              ...
              {areaScores.map((area) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span
                        ...
                        style={{ color: areaScoreToColor(area.average) }}
                      >
                  ...
                  <ProgressBar
                    ...
                    color={areaScoreToColor(area.average)}
                    height={6}
                  />
                  {hoveredArea === area.areaId && (
                    <p className="text-xs text-teal-600 mt-1">{Math.round(area.pctLevel2Plus)}% auf Level ≥ 2</p>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {strengths.map((area, i) => (
                <div key={area.areaId} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-700 truncate">{area.areaTitle}</p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-base font-semibold text-gray-800">Entwicklungspotenzial</h2>
            </div>
            <div className="space-y-3">
              {gaps.map((area) => (
                <div key={area.areaId} className="flex items-center gap-3">
